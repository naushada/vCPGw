...
public:
  ...
  JSONValue *json_new_value(int i);
  JSONValue *json_new_value(double d);
  JSONValue *json_new_value(char *str);
  JSONValue *json_new_value(char *str, int size);
  JSONValue *json_new_value(JSONObject *object);
  JSONValue *json_new_value(JSONArray *array);
  JSONValue *json_new_value(const ACE_UINT8 tOrF);
  JSONValue *json_new_value(std::nullptr_t nullp);

  JSONElement *json_new(JSONValue *value);
  JSONArray *json_new(JSONElement *element);
  JSONMember *json_new(JSONValue *key, JSONValue *value);
  JSONObject *json_new(JSONMember *member);
